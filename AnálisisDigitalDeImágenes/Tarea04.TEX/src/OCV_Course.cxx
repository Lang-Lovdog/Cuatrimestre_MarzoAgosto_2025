...
int lovdog_Histograma(const char* title, const cv::Mat& input){
  ...
  for( unsigned int i = 1 ; (int)i < histSize ; ++i)
    line(
        histogramaImg,
...
int lovdogCalcHist(const cv::Mat& src, cv::Mat &histogram1D, bool cnt_pdf, bool std_cum){
  ...
  switch(src.type()){
    case CV_8UC1:
      for(size_t theRow=0; (int)theRow < src.rows ; ++theRow)
        for(size_t theCol=0; (int)theCol<src.cols; ++theCol)
...
int lovdogShowHist(const cv::Mat& histogram, cv::Mat& ploutput, size_t width, size_t height){
  ...
  for(size_t theCol=1; (int)theCol<histogram.cols; ++theCol)
  ...
  for( unsigned int i = 1 ; (int)i < histogram.cols ; ++i)
    line(
        ploutput,
...
int CannyMain(int argc, char** argv){
  ...
  i=0;
  while((int)i < (  lImage_f.rows*lImage_f.cols)) {i+=lImage_f.channels(); *(lImage_f.data+i)=i%256;}
  while((int)i < (2*lImage_f.rows*lImage_f.cols)) {i+=lImage_f.channels(); *(lImage_f.data+i+1)=i%256;}
  while((int)i < (3*lImage_f.rows*lImage_f.cols)) {i+=lImage_f.channels(); *(lImage_f.data+i+2)=i%256;}
...
int mainSession03(int argc, char** argv){
  ...
  showIt(lImage,"Original Image",false);
  ...
  //threshold(lImageG,lImageT,80,200,cv::THRESH_OTSU);
  showIt(lImageT,"Original Image Gray Threshold",false);
  Contornos contornos;
  lovdogExtraeContornos(lImageT, contornos);
  lovdogDibujaContornos(lImageT.size(), contornos,lImageC);
  showIt(lImageC,"Original Image Contornos",false);
...
int lovdogExtraeContornos(cv::Mat ImagenEntrada, Contornos& contornos){
  cv::findContours(ImagenEntrada,contornos,cv::RETR_TREE,cv::CHAIN_APPROX_NONE);
  return 0;
}

int lovdogDibujaContornos(cv::Size tamannoOriginal, Contornos contornos,cv::Mat& lImageC){

  cv::RNG rng(1673);

  lImageC = cv::Mat::zeros(tamannoOriginal, CV_8UC3);
  cv::Scalar color;

  for(size_t i=0; i<contornos.size(); ++i){
    color = cv::Scalar(rng.uniform(0, 255), rng.uniform(0, 255), rng.uniform(0, 255));
    cv::drawContours(lImageC,contornos,(int)i,color,1);
  }
  return 0;
}
