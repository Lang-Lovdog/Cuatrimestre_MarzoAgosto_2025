...
class ModelSelection:
    ...
    def __init__(self, name, csv=None, gridParams=None, classifiers=None):
        if name in _ModelSelectionClassesNames_:
            raise ValueError(f"Object '{name}' already exists")
            return
        if classifiers is None and gridParams is not None:
            raise ValueError("You must specify classifiers")
            return
        if gridParams is None and classifiers is not None:
            raise ValueError("You must specify gridParams")
            return
        for grparamKey in gridParams.keys():
            if grparamKey not in classifiers.keys():
                raise ValueError(f"Classifier '{grparamKey}' does not exist")
                return
        ...
        if gridParams is None:
            ...
            self.gridParameters = gridParams
...
class FeatureSelectionEDA:
    ...
    def _edas_variation(self, population, fraction=0.3):
        """
        Creates a new population by sampling from a probability model
        built from the top performers in the current population.
        """
        # 1. Select the elite individuals (the best fraction of the population)
        elites = tools.selBest(population, k=int(len(population) * fraction))
        
        # 2. Convert the elite list to a NumPy array for easy calculation
        elite_array = np.array(elites)
        
        # 3. Build the probabilistic model: Calculate the probability of each feature being '1'
        #    This is the mean of each column (feature) in the elite set.
        feature_probs = np.mean(elite_array, axis=0)
        
        # 4. Avoid extremes to prevent premature convergence
        #    This keeps probabilities away from 0.0 and 1.0, maintaining diversity.
        feature_probs = np.clip(feature_probs, 0.1, 0.9)
        
        # 5. Generate new individuals by sampling from the probability model
        new_population = []
        for _ in range(len(population)):
            # Sample each feature independently based on its probability
            new_ind = self.toolbox.individual()
            for j in range(self.n_features):
                if random.random() < feature_probs[j]:
                    new_ind[j] = 1
                else:
                    new_ind[j] = 0
    ...
    def run(self):
        ...
        for gen in range(self.EDAS_Config["num_generations"]):
            ...
            elite_size = int(self.EDAS_Config["population_size"] * self.EDAS_Config["elite_fraction"])
            elites = tools.selBest(population, k=elite_size)

            # 2. Create new population using EDAS variation
            new_population = self._edas_variation(population, fraction=elite_size / len(population))

            # 3. Evaluate new individuals
            new_fitnesses = list(map(self.toolbox.evaluate, new_population))
            for ind, fit in zip(new_population, new_fitnesses):
                ind.fitness.values = fit
